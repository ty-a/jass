<?php
	...
	function deleteSubmission( $submissionID, $db_handler ) {
		...
		if(!($prepared_statement = $db_handler->prepare("DELETE FROM submissions WHERE submissionID = $submissionID"))) {
			displayErrorMessage("Database error, please try again later.");
			return false;
		} // end prepare sql statement 
		
		// 's' shows that $username is a string
		//if(!($prepared_statement->bind_param( 'sss', $submissionID, $_SESSION['userId'] ))) {
		//	displayErrorMessage("Database error, please try again later.");
		//	return false;
